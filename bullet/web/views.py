...
class HomepageView(TemplateView, BranchSpecificViewMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['competition'] = Competition.objects.currently_active().get(branch=self.branch)
        context['registration'] = Competition.objects.currently_running_registration().get(branch=self.branch)
        return context
...
class RegistrationView(FormView, BranchSpecificViewMixin):
    ...
    def get_competition_model_instances(self):
        try:
            ...
            raise ValueError(f'No {self.branch} competition has an open registration now')
        ...
        try:
            ...
            raise ValueError(f'{self.kwargs["category"]} category cannot compete in {competition}')
    ...
    def dispatch(self, request, *args, **kwargs):
        try:
            ...
            return HttpResponseRedirect(reverse('homepage', host=self.branch.label.lower()))
    ...
    def forms_valid(self, form, formset):
        ...
        team.secret_link = ''.join(random.choices(string.ascii_letters + string.digits, k=48))
        team.save()
...
class RegistrationConfirmView(TemplateView, BranchSpecificViewMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            team = Team.objects.select_related('competition_site__category_competition__competition').get(
